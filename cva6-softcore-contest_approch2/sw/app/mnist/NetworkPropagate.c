    ...
    int OUTPUT_MEM_STRIDE)
{
    ...
    for (int oy = 0; oy < OUTPUTS_HEIGHT; ++oy) {
        ...
        const int syMax = (PADDING_Y == 0 //syMax is always KERNEL_HEIGHT
        ...
        for (int ox = 0; ox < OUTPUTS_WIDTH; ++ox) {
            for (int output = 0; output < NB_OUTPUTS; ++output) { //accoding to the number of output to define the loop number
                ...
                const int sxMax = (PADDING_X == 0 //sxMax is always KERNEL_WIDTH
                ...
                for (int sy = 0; sy < KERNEL_HEIGHT; ++sy) { //in the kernel, start by line
                    if ((PADDING_Y != 0
                        ...
                        && sy >= syMax - syMin)
                    {
                        break; // when there is padding and sy surpass the valid field
                    }
