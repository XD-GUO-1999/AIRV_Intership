...
#(
    ...
    input  riscv::xlen_t                     operand_g_i,
    input  riscv::xlen_t                     operand_h_i,
    input  riscv::xlen_t                     operand_i_i,
    ////
  ...
  //modification the logic calculate of MAC16
  logic [31:0] mac16_res_d, mac16_res_q;
...
//modification to debug
  ...
  //modification logic of calculation
    assign mac16_res_d = ($signed({1'b0, operand_a_i[7:0]})*$signed(operand_b_i[7:0])) + 
                          ...
                          ($signed({1'b0, operand_f_i[7:0]})*$signed(operand_g_i[7:0])) + 
                          ($signed({1'b0, operand_f_i[15:8]})*$signed(operand_g_i[15:8])) + 
                          ($signed({1'b0, operand_f_i[23:16]})*$signed(operand_g_i[23:16])) + 
                          ($signed({1'b0, operand_f_i[31:24]})*$signed(operand_g_i[31:24])) +
                          ($signed({1'b0, operand_h_i[7:0]})*$signed(operand_i_i[7:0])) + 
                          ($signed({1'b0, operand_h_i[15:8]})*$signed(operand_i_i[15:8])) + 
                          ($signed({1'b0, operand_h_i[23:16]})*$signed(operand_i_i[23:16])) + 
                          ($signed({1'b0, operand_h_i[31:24]})*$signed(operand_i_i[31:24])) +
                          $signed(operand_c_i); 
  ...
  assign mult_valid      = mult_valid_i && (operation_i inside {MUL, MULH, MULHU, MULHSU, MULW, CLMUL, CLMULH, CLMULR, ariane_pkg::MAC16});
  ...
  always_comb begin : p_selmux
    ...
    unique case (operator_q)
      ariane_pkg::MAC16:    result_o = mac16_res_q; //modification: output of mac16
