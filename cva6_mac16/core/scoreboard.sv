...
module scoreboard #(
    ...
    input  logic         [ariane_pkg::REG_ADDR_SIZE-1:0] rs6_i,
    output riscv::xlen_t                                 rs6_o,
    output logic                                         rs6_valid_o,

    input  logic         [ariane_pkg::REG_ADDR_SIZE-1:0] rs7_i,
    output riscv::xlen_t                                 rs7_o,
    output logic                                         rs7_valid_o,

    input  logic         [ariane_pkg::REG_ADDR_SIZE-1:0] rs8_i,
    output riscv::xlen_t                                 rs8_o,
    output logic                                         rs8_valid_o,

    input  logic         [ariane_pkg::REG_ADDR_SIZE-1:0] rs9_i,
    output riscv::xlen_t                                 rs9_o,
    output logic                                         rs9_valid_o,
    /////
  ...
  for (genvar k = 0; unsigned'(k) < CVA6Cfg.NrWbPorts; k++) begin : gen_rs_wb
    ...
    //modification, check the write back of rs6 and rs7
    assign rs6_fwd_req[k] = (mem_q[trans_id_i[k]].sbe.rd == rs6_i) & wt_valid_i[k] & (~ex_i[k].valid) & (~mem_q[trans_id_i[k]].is_rd_fpr_flag);
    assign rs7_fwd_req[k] = (mem_q[trans_id_i[k]].sbe.rd == rs7_i) & wt_valid_i[k] & (~ex_i[k].valid) & (~mem_q[trans_id_i[k]].is_rd_fpr_flag);
    //modification, check the write back of rs8 and rs9
    assign rs8_fwd_req[k] = (mem_q[trans_id_i[k]].sbe.rd == rs8_i) & wt_valid_i[k] & (~ex_i[k].valid) & (~mem_q[trans_id_i[k]].is_rd_fpr_flag);
    assign rs9_fwd_req[k] = (mem_q[trans_id_i[k]].sbe.rd == rs9_i) & wt_valid_i[k] & (~ex_i[k].valid) & (~mem_q[trans_id_i[k]].is_rd_fpr_flag);
...
//////////
  ...
  for (genvar k = 0; unsigned'(k) < NR_ENTRIES; k++) begin : gen_rs_entries
    ...
    assign rs7_fwd_req[k+CVA6Cfg.NrWbPorts] = (mem_q[k].sbe.rd == rs7_i) & mem_q[k].issued & mem_q[k].sbe.valid & (~mem_q[k].is_rd_fpr_flag);
    assign rs8_fwd_req[k+CVA6Cfg.NrWbPorts] = (mem_q[k].sbe.rd == rs8_i) & mem_q[k].issued & mem_q[k].sbe.valid & (~mem_q[k].is_rd_fpr_flag);
    assign rs9_fwd_req[k+CVA6Cfg.NrWbPorts] = (mem_q[k].sbe.rd == rs9_i) & mem_q[k].issued & mem_q[k].sbe.valid & (~mem_q[k].is_rd_fpr_flag);
...
//modification: check if we are in x0 for s4 s5
  ...
  assign rs7_valid_o = rs7_valid & (|rs7_i);
  assign rs8_valid_o = rs8_valid & (|rs8_i);
  assign rs9_valid_o = rs9_valid & (|rs9_i);
////
  // use fixed prio here
