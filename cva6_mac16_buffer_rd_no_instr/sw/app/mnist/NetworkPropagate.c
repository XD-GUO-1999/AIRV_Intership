        ...
        for (int ox = 0; ox < OUTPUTS_WIDTH; ++ox) {
            ...
            for (int output = 0; output < NB_OUTPUTS; ++output) {
                SUM_T weightedSum = biasses[output];
                     ...
                     */
                    // first block: sy=0, sx=0
                    {
                        const int sy = 0;
                        const int sx = 0;

                        const int wOffset = NB_CHANNELS * (
                            sxMin + sx
                            + KERNEL_WIDTH * (syMin + sy + KERNEL_HEIGHT * output)
                        );

                        mac16buf_first(weights + wOffset, &weightedSum);
                    }

                    // middle: sy=0, sx=1..4
                    for (int sx = 1; sx < KERNEL_WIDTH; ++sx) {
                        const int sy = 0;
                    ...
                    // middle: sy=1..3, sx=0..4
                    for (int sy = 1; sy < KERNEL_HEIGHT - 1; ++sy) {
                        for (int sx = 0; sx < KERNEL_WIDTH; ++sx) {
                            const int wOffset = NB_CHANNELS * (
                                sxMin + sx
                                + KERNEL_WIDTH * (syMin + sy + KERNEL_HEIGHT * output)
                    ...
                    // middle: sy=4, sx=0..3
                    for (int sx = 0; sx < KERNEL_WIDTH - 1; ++sx) {
                        const int sy = KERNEL_HEIGHT - 1;
                    ...
                    // final block: sy=4, sx=4
                    {
                        const int sy = KERNEL_HEIGHT - 1;
                        const int sx = KERNEL_WIDTH - 1;
                        ...
                        mac16buf_final(weights + wOffset, &weightedSum);
                    }
                outputs[oOffset + output]
                    = sat(weightedSum, output, ACTIVATION, rescaling);
            }
    ...
    int OUTPUT_MEM_STRIDE)
{
    ...
    const int total_inputs = NB_CHANNELS * CHANNELS_WIDTH * CHANNELS_HEIGHT;
        ...
        for (int och = 0; och < NB_OUTPUTS; och++) {
            ...
            for (int block = 1; block < 23; ++block) {
